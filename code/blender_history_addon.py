...
class HISTORY_OT_restore_state(Operator):
    ...
    def execute(self, context):
        ...
        if history_props.active_history_index < 0 or history_props.active_history_index >= len(history_props.history_list):
...
class HISTORY_OT_delete_entry(Operator):
    ...
    def execute(self, context):
        ...
        if history_props.active_history_index >= 0 and history_props.active_history_index < len(history_props.history_list):
...
class VIEW3D_PT_history_manager(Panel):
    ...
    def draw(self, context):
        ...
        if history_props.auto_record:
            row.operator("history.auto_record_toggle", text="Stop Auto Record", icon='PAUSE')
        else:
            row.operator("history.auto_record_toggle", text="Start Auto Record", icon='REC')
        ...
        layout.template_list(
            "HISTORY_UL_list", "",
            history_props, "history_list",
            history_props, "active_history_index",
            rows=5
        )
...
def auto_record_handler(scene):
    history_props = scene.history_manager
    if history_props.auto_record and bpy.context.active_object and bpy.context.active_object.type == 'MESH':
        # 一定間隔で自動保存（この例では簡略化）
        pass
...
def register():
    ...
    bpy.types.Scene.history_manager = bpy.props.PointerProperty(type=HistoryManagerProperties)
